...
func GenerateHintExpr(usedTables []gmodel.Table) (h *ast.TableOptimizerHint) {
	...
	for _, t := range usedTables {
		for _, i := range t.Index() {
			if tableHasIndex[t.Name()] == nil {
				tableHasIndex[t.Name()] = make([]string, 0)
			}
			tableHasIndex[t.Name()] = append(tableHasIndex[t.Name()], i.Column.Name.String())
		}
	}
	...
	hintKeyword := enabledKeywords[rand.Intn(len(enabledKeywords))]
	...
	for _, t := range usedTables {
		shuffledTables = append(shuffledTables, ast.HintTable{
			TableName: model.NewCIStr(t.Name()),
		})
	}
	...
	case "hash_join", "merge_join", "inl_join", "inl_hash_join", "inl_merge_join":
		...
		n := mathutil.Min(rand.Intn(4)+2, len(shuffledTables)) // avoid case n < 2
		for ; n > 0; n-- {
	...
	case "use_index", "ignore_index", "use_index_merge":
		...
		n = mathutil.Min(rand.Intn(4)+1, len(indexes)) // avoid case n == 0
		for ; n > 0; n-- {
			idx := indexes[n-1]
